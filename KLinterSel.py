    ...
    if STATS:
        ...
        for i in range(numfiles):
            ...
            for c in range(numcroms):
                ...
                print("Mean:", np.mean(datos_filtrados[i][c]))
                print("SD:", np.std(datos_filtrados[i][c]))
                print(f'Mediana {np.percentile(datos_filtrados[i][c],50)}')
        ...
        for c in range(numcroms):
            ...
            if nonEmpty:
                ...
                for r in range(numrs):
                    ...
                    #avertot_distances =avertot_distances +rtot_distances[-1]
                ...
                #avertot_distances = np.sort(np.mean(np.vstack(rtot_distances), axis=0))
                
                
                if not RANDOM: #Divergencia entre la distribución observada y la media
